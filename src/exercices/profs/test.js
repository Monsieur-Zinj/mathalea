...
function comparaisonFractionArnoG (input, goodAnswer) {
  ...
  // console.info(saisie.op1, saisie.op1.value, 'saisie.op1.re', saisie.op1.re, 'reponse.op1.re', reponse.op1.re)
  console.info(saisie.isInteger, reponse.isInteger)
  if (saisie.isEqual(reponse)) {
    ...
    if (!saisie.op1.value && !reponse.isInteger) { // Si pas de op1.value, c'est que saisie est un nombre
      console.info(false, 'Résultat incorrect car une fraction est attendue.') // Sous-entendu : Et pas un nombre (sauf si la fraction se réduit à un entier)
    } else if (saisie.isInteger && reponse.isInteger) console.info(true)
    else if (
      saisie.operator === 'Divide' ||
    (saisie.operator === 'Rational' &&
      saisie.op1.re < reponse.op1.re &&
      reponse.op1.re % saisie.op1.re === 0)
    ) { console.info(true) } else if (
      saisie.operator === 'Divide' ||
    ...
    } else console.info(false, 'Résultat incorrect car une fraction est attendue2.')
  } else console.info(false, 'Résultat incorrect.')
}
...
function comparaisonFraction (input, goodAnswer) {
  ...
  console.info(reponseNativeParsed, reponseNativeParsed.value, reponseParsed)
  ...
  reponseNativeParsed.op1.value % saisieNativeParsed.op1.value !== 0) { // reponse doit avoir des numérateur/dénominateur multiples de ceux de saisie
    ...
    /// Ci-dessous : le traitement des feedback : les saisies sont égales aux réponses mais la saisie ne convient tout de même pas.
      ...
      console.info(false, 'Résultat incorrect car une fraction est attendue.') // Sous-entendu : Et pas un nombre
      ...
      if (saisieNativeParsed.op1.value >= reponseNativeParsed.op1.value) {
        console.info(false, 'Résultat incorrect car une fraction simplifiée est attendue.') // Sous-entendu : Et pas numérateur/dénominateur plus grands ou égaux que reponse
      } else if (saisieNativeParsed.op1.isInteger && saisieNativeParsed.op2.isInteger) {
        console.info(false, 'Résultat incorrect car dénominateur et numérateur doivent être entiers.') // Sous-entendu : Et pas numérateur/dénominateur décimaux pour au moins l'un d'entre eux
      } else if (reponseNativeParsed.op1.value % saisieNativeParsed.op1.value !== 0) {
        console.info(false, 'Résultat incorrect car une fraction réduite est attendue.') // Sous-entendu : Et pas numérateur/dénominateur de reponse non multiples de ceux de saisie
      }
    } else console.info(false, 'Résultat incorrect car une fraction est attendue.') // Sous-entendu : Et pas une opération autre qu'une division
  } else console.info(false, 'Résultat incorrect.')
}
// console.info('test OOOO')
// comparaisonFractionArnoG('\\frac{12}{6}', '\\frac{24}{12}') // -> true
// console.info('test XXXX11')
// comparaisonFraction('-\\frac{24}{12}', '\\frac{-24}{12}') // -> true
// console.info('test YYYYY')
// comparaisonFraction('\\frac{24}{-12}', '\\frac{-24}{12}') // -> true
