...
class ConstructionRectangleDimensions extends Exercice {
  ...
  correctionInteractive = () => {
    ...
    const { isValid: isValid4, message: message4 } = this.figure.testDistance({ label1: 'A', label2: 'B', distance: this.L })
    resultat.push(isValid4 ? 'OK' : 'KO')
    if (message4 !== '') { feedback += message4 + '<br>' }
    const { isValid: isValid5, message: message5 } = this.figure.testDistance({ label1: 'B', label2: 'C', distance: this.l })
