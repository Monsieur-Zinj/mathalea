...
import { choice, compteOccurences, enleveElement } from '../../lib/outils/arrayOutils.js'
...
import { point, segment, tracePoint, homothetie, polygone, symetrieAxiale, translation, droite, vecteur, rotation, milieu, texteParPointEchelle } from '../../modules/2d.js'
      ...
      for (let etape = 0; etape < chemin.length - 1; etape++) {
        texte += (this.interactif && context.isHtml)
          ...
          : (etape === 0)
              ...
              : this.sup3
                ? `$${miseEnEvidence(etape + 1 + ')' + sp(1))}$` + texteEnCouleur('qui ' + transfos[etape].texte.substr(context.isHtml ? 22 : 18) + (etape === chemin.length - 2 ? '.' : ','), etape % 2 === 0 ? 'black' : 'brown') + '<br>'
