...
export const titre = 'Périmètre du rectangle'
...
export default class PerimetreRectangle extends Exercice {
  ...
  nouvelleVersion () {
    ...
    this.canEnonce = mathalea2d(Object.assign({ scale: grandeBase > 5 ? 0.75 : 1, style: 'display: block', pixelsParCm: grandeBase > 5 ? 20 : 30 }, fixeBordures(objets)), objets)
    this.canReponseACompleter = 'Le périmètre de ce rectangle est égal à $\\ldots$ cm.'
    this.reponse = String((hauteur + grandeBase) * 2)
    this.correction = `Le périmètre du rectangle est égal à : $(${String(grandeBase)}+${String(hauteur)})\\times 2=${String(grandeBase + hauteur)}\\times 2 = ${miseEnEvidence(this.reponse)}$.<br>`
  }
}
