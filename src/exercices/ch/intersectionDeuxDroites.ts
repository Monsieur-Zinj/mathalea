...
export default class systemeEquationsPremDegComp extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      const listeVarPoint = ['A', 'B', '', 'A', 'B', '']
      if (this.correctionDetaillee) {
        texteCorr = texteCorr + 'S\'il existe, on note $(A; B)$ le point d\'intersection des deux droites. On obtient le système suivant qu\'on résout par comparaison :'
        ...
        if (listeVarPoint[(valComp + 1) % 2] === 'A') {
