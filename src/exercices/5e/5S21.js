...
import { listeQuestionsToContenu, randint, combinaisonListes, numAlpha, range, sp } from '../../modules/outils.js'
  ...
  this.nouvelleVersion = function () {
    ...
    for (let i = 0, p, m, q, somme, quidam, index1, lieu, objet, objets, article, pronom, defini, n = [], texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      if (parseInt(this.niveau) === 1) {
        ...
        texteCorr += numAlpha(0) + ` Il y a ${n[m]} ${objets} ${qualites[index1][m]} et il y a ${somme} ${objets} possibles. La probabilité que son choix tombe sur l'${article} des ${objets} ${qualites[index1][m]} est :<br> $${deprecatedTexFraction(n[m], somme)}${simplificationDeFractionAvecEtapes(n[m], somme)}$.<br>`
        texteCorr += numAlpha(1) + ` Il y a ${n[p]} ${objets} ${qualites[index1][p]} et il y a ${somme} ${objets} possibles. La probabilité que son choix tombe sur l'${article} des ${objets} ${qualites[index1][p]} est :<br> $${deprecatedTexFraction(n[p], somme)}${simplificationDeFractionAvecEtapes(n[p], somme)}$.<br>`
        texteCorr += numAlpha(2) + ` Il y a ${n[q]} ${objets} ${qualites[index1][q]}, donc il y a ${somme} $-$ ${n[q]} $=$ ${somme - n[q]} autres ${objets} et il y a ${somme} ${objets} possibles. La probabilité que son choix ne tombe pas sur l'${article} des ${objets} ${qualites[index1][q]} est :<br> $${deprecatedTexFraction(somme - n[q], somme)}${simplificationDeFractionAvecEtapes(somme - n[q], somme)}$.<br>`
        texteCorr += numAlpha(3) + ` La probabilité d'un événement est la somme des probabilités des issues qui le composent. Donc la probabilité que son choix tombe sur l'${article} des ${objets} ${qualites[index1][m]} ou ${qualites[index1][p]} est :<br> $${deprecatedTexFraction(n[m], somme)}+${deprecatedTexFraction(n[p], somme)}=${deprecatedTexFraction(n[p] + n[m], somme)}${simplificationDeFractionAvecEtapes(n[p] + n[m], somme)}$.<br>`
      } else {
