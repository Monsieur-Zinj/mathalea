...
import { listeQuestionsToContenu, randint, choice, numAlpha, rienSi1, ecritureAlgebrique } from '../../modules/outils.js'
...
export default class EtudeTrinome extends Exercice {
  ...
  nouvelleVersion () {
    ...
    let correction1 = `$\\begin{aligned}
    f(x)&=${p.texFormeCanonique}\\\\
   &=${etapesDeveloppement[0]}\\\\
...
\\iff &${a !== 1 ? `${p.a.simplifie().texFractionSaufUn}\\left( x ${p.alpha.oppose().simplifie().texFractionSignee} \\right)^2${a > 0 ? '\\geqslant 0' : '\\leqslant 0'}\\\\` : ''}
\\iff &${p.texFormeCanonique}${a > 0 ? '\\geqslant' : '\\leqslant'}${p.beta.simplifie().texFraction}\\\\
\\iff & f(x)${a > 0 ? '\\geqslant' : '\\leqslant'}${p.beta.simplifie().texFractionSignee}
\\end{aligned}$
<br>
