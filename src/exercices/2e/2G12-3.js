...
import { axes, repere, grille } from '../../lib/2d/reperes.js'
...
export default function Parallélogramme () {
  ...
  this.nouvelleVersion = function () {
    ...
    for (let i = 0, a, ux, uy, g, XMIN, XMAX, YMIN, YMAX, test, objets,nom,  J, o, s1, s2, s3, s4, s5, s6, AB2, AC2, BC2, DB2, DC2, xA, yA, xB, yB, xC, yC, xD, yD, xM, yM, xN, yN, xI0, xI1, yI0, yI1, xJ0, xJ1, yJ0, yJ1, A, B, C, D, P, T, L, M, N, I, O, texte, texteCorr, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      // Combien de chiffres ? Quelles valeurs ?
       ...
       `
          ...
          objets.push(repere({ xMin: XMIN, yMin: YMIN, xMax: XMAX, yMax: YMAX, yLabelEcart: 0.6, xLabelEcart: 0.6,yLabelDistance: 2,
            xLabelDistance: 2 }))
            ...
            texteCorr += `<br> Soit $M$ le milieu de $[${A.nom}${D.nom}]$ : <br> `
                       texteCorr += `$\\begin{cases}x_M=\\dfrac{x_${A.nom}+x_${D.nom}}{2}= \\dfrac{${xA}+${xD}}{2}=\\dfrac{${texNombre(xA + xD)}}{2}${xM.texSimplificationAvecEtapes()}\\\\[0.5em] y_M=\\dfrac{y_${A.nom}+y_${D.nom}}{2}= \\dfrac{${yA}+${yD}}{2}=\\dfrac{${texNombre(yA + yD)}}{2}${yM.texSimplificationAvecEtapes()}\\end{cases}$`
           
           texteCorr += `  <br>Ainsi : $ M\\left(${xM.simplifie().texFSD}\\,;\\,${yM.simplifie().texFSD}\\right)$` 
           texteCorr +=`<br> Soit $N$ le milieu de $[${B.nom}${C.nom}]$ : <br> `
                       texteCorr += `$\\begin{cases}x_N=\\dfrac{x_${B.nom}+x_${C.nom}}{2}= \\dfrac{${xB}+${xC}}{2}=\\dfrac{${texNombre(xB + xC)}}{2}${xN.texSimplificationAvecEtapes()}\\\\[0.5em] y_N=\\dfrac{y_${B.nom}+y_${C.nom}}{2}= \\dfrac{${yB}+${yC}}{2}=\\dfrac{${texNombre(yB + yC)}}{2}${yN.texSimplificationAvecEtapes()}\\end{cases}$`
           
           texteCorr += `  <br>Ainsi : $ N\\left(${xN.simplifie().texFSD}\\,;\\,${yN.simplifie().texFSD}\\right)$`
