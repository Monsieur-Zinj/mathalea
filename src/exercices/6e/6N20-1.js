...
export default class EncadrerFractionEntre2Entiers extends Exercice {
  ...
  nouvelleVersion () {
    ...
    for (let i = 0, texte, texteCorr, n, d, k, cpt = 0; i < this.nbQuestions && cpt < 50;) {
      ...
      // texte += ajouteFeedback(this, i) remplisLesBlancs() a déjà son divFeedback.
        ...
        if (context.isAmc) {
          ...
          handleAnswers(this, i, {
            ...
            feedback: (saisies/** {champ1: string, champ2:string} */) => {
              ...
              const { feedback } = consecutifsCompare(`${rep1}<${(n / d).toFixed(4)}<${rep2}`, { entierInf: a, entierSup: b, valeurInter: (a + b) / 2 })
              return feedback
            },
