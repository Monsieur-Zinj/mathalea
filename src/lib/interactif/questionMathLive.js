...
export function remplisLesBlancs (exercice, question, content, classes, blanc = '\\ldots') {
  ...
  if (exercice.interactif) {
    return `<math-field readonly style="font-size:2em" class="fillInTheBlanks" id="champTexteEx${exercice.numeroExercice}Q${question}">${mfeValue}</math-field><span id="resultatCheckEx${exercice.numeroExercice}Q${question}"></span><span class=${classes} id="feedbackEx${exercice.numeroExercice}Q${question}"></span>`
  } else {
    return `$${mfeValue}$`
  }
}
