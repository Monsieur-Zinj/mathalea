...
<main class="bg-coopmaths-canvas dark:bg-coopmathsdark-canvas {$darkMode.isActive ? 'dark' : ''}">
  ...
  <section class="px-4 py-0 md:py-10 bg-coopmaths-canvas dark:bg-coopmathsdark-canvas">
    ...
    <p class="text-coopmaths-corpus dark:text-coopmathsdark-corpus text-lg md:text-xl">
      MathALÉA vous permet d'importer automatiquement les exercices sélectionné dans Anki.<br />
      Cette interface nécessite que <a href="https://apps.ankiweb.net/" target="_blank"
      rel="noopener noreferrer" class="text-coopmaths-action dark:text-coopmathsdark-action">Anki</a> soit installé sur votre ordinateur avec le plugin <a href="https://ankiweb.net/shared/info/2055492159" target="_blank"
      rel="noopener noreferrer" class="text-coopmaths-action dark:text-coopmathsdark-action">AnkiConnect</a> et que Anki soit <strong>actuellement lancé</strong> sur votre ordinateur.<br />Veuillez consulter la <a
    ...
    <h1 class="mt-12 mb-4 text-center md:text-left text-coopmaths-struct dark:text-coopmathsdark-struct text-2xl md:text-4xl font-bold">Importation</h1>
    ...
    <div class="flex flex-row w-full justify-center md:justify-start items-center">
      {#if logs}
        <pre class="my-10 shadow-md bg-coopmaths-canvas-dark dark:bg-coopmathsdark-canvas-dark text-coopmaths-corpus dark:text-coopmathsdark-corpus p-4 w-full overflow-auto">{logs}</pre>
      {/if}
    </div>
  </section>
  <footer>
