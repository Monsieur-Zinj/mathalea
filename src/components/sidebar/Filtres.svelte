...
<div class="{isVisible ? 'flex' : 'hidden'} flex-col px-4">
  ...
  <ul>
    {#each Array.from(typesMap.keys()) as key}
      <li>
        <div class="flex-row justify-start items-center pr-4 pl-6">
          <input
            id="checkbox-{key}"
            aria-describedby="checkbox-{key}"
            type="checkbox"
          ...
          />
          <label for="checkbox-{key}" class="ml-2 text-xs font-light text-coopmaths-corpus dark:text-coopmathsdark-corpus"> {typesMap.get(key).title} </label>
        </div>
      </li>
    {/each}
  </ul>
  ...
  <ul>
    {#each Array.from(levelsMap.keys()) as key}
      <li>
        <div class="flex-row justify-start items-center pr-4 pl-6">
          <input
            id="checkbox-{key}"
          ...
          />
          <label for="checkbox-{key}" class="ml-2 text-xs font-light text-coopmaths-corpus dark:text-coopmathsdark-corpus"> {levelsMap.get(key).title} </label>
        </div>
      </li>
    {/each}
  </ul>
