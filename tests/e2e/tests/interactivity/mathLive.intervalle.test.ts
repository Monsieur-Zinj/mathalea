...
async function testSalaires (page: Page) {
  ...
  for (const question of questions) {
    ...
    const arrayNumber = array.filter((value) => Number.isInteger(parseFloat(value)))
...
async function testNotes (page: Page) {
  ...
  for (const question of questions) {
    ...
    const notes = unorderedNotes.map((note) => Number(note)).sort((a, b) => a - b)
    if (question.isCorrect) {
      if (notes.length % 2 === 0) {
        if (notes[notes.length / 2 - 1] === notes[notes.length / 2]) {
          ...
          reponse = String((notes[notes.length / 2 - 1] + notes[notes.length / 2]) / 2 + 0.1)
        }
      } else {
